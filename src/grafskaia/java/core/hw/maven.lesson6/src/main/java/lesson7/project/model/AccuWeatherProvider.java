...
//http://dataservice.accuweather.com/locations/v1/cities/search
    ...
    @Override
    public List<WeatherObject> getWeather(Period period) throws RuntimeException {
            ...
            try {
                ...
                if (objectMapper.readTree(weatherResponse).size() > 0) {
                 //   makeWeatherForecast(weatherResponse);
                    for (WeatherObject wo: createWeatherObjectForDb(weatherResponse)) {
                        weatherObjects.add(wo);
                    }
                    } else {
...
//    }
    ...
    private List<WeatherObject> createWeatherObject(WeatherResponse weatherResponse) throws ParseException {
        ...
        for (DailyForecasts i: weatherResponse.getDailyForecasts()) {
            WeatherObject wo = new WeatherObject(GlobalState.getInstance().getSelectedCity(), changeDateFormat(i.getDate()),i.getDay().getDescription().toLowerCase(Locale.ROOT),i.getNight().getDescription().toLowerCase(Locale.ROOT),i.getTempInfo().getMax().getTemperatureValue(),i.getTempInfo().getMin().getTemperatureValue());
            weatherObject.add(wo);
        }
        return weatherObject;
    }

    private List<WeatherObject> createWeatherObjectForDb(String jsonResponse) throws JsonProcessingException, ParseException {
        ObjectMapper objectMapper = new ObjectMapper();
        objectMapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);
        WeatherResponse weatherResponse = objectMapper.readValue(jsonResponse,new TypeReference<WeatherResponse>() {});
        List<WeatherObject> weatherObjects = createWeatherObject(weatherResponse);
        return weatherObjects;
    }
}
